...
def calculate_features(mfccs):
    ...
    return tf.concat([tf.expand_dims(mfccs, axis=-1), tf.expand_dims(delta, axis=-1), tf.expand_dims(delta_delta, axis=-1)], axis=-1)
...
# 모델
def get_model():
    input_shape = (int((wav_sampling_rate * wav_max_length - frame_length) / frame_step) + 1, num_mfccs, 3)
    title_input = tf.keras.Input(
        shape=input_shape, name="input"
    )

    resnet = tf.keras.applications.efficientnet.EfficientNetB5(weights=None, include_top=False, input_shape=input_shape, pooling="max")(title_input)
    
    #lstm = layers.LSTM(128)(lstm)
    age_dense = layers.Dense(512, activation='relu')(resnet)
    ...
    dialect_dense = layers.Dense(512, activation='relu')(resnet)
    ...
    gender_dense = layers.Dense(512, activation='relu')(resnet)
