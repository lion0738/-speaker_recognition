...
lower_edge_hertz, upper_edge_hertz, num_mel_bins, num_mfccs = 40.0, 7600.0, 80, 13
...
def calculate_features(mfccs):
    ...
    data = tf.expand_dims(tf.concat([mfccs, delta, delta_delta], axis=-1), axis=-1)

    return tf.concat([data, data, data], axis=-1)
...
# 모델
def get_model():
    input_shape = (int((wav_sampling_rate * wav_max_length - frame_length) / frame_step) + 1, num_mfccs * 3, 3)
    title_input = tf.keras.Input(
        shape=input_shape, name="input"
    )

    volo_model = volo.VOLO_d5(input_shape=input_shape, pretrained=None)(title_input)
    dense = layers.Dense(512, activation='relu')(volo_model)
    #tf.keras.applications.efficientnet.EfficientNetB7(weights=None, include_top=False, input_shape=input_shape, pooling="max")(title_input)
    
    #lstm = layers.LSTM(128)(lstm)
    age_dense = layers.Dense(512, activation='relu')(dense)
    ...
    dialect_dense = layers.Dense(512, activation='relu')(dense)
    ...
    gender_dense = layers.Dense(512, activation='relu')(dense)
