...
lower_edge_hertz, upper_edge_hertz, num_mel_bins, num_mfccs = 40.0, 7600.0, 80, 32
...
def calculate_features(mfccs):
    ...
    return tf.expand_dims(tf.concat([mfccs, delta, delta_delta], axis=-1), axis=-1)
...
# 모델
def get_model():
    input_shape = (int((wav_sampling_rate * wav_max_length - frame_length) / frame_step) + 1, num_mfccs, 1)
    title_input = tf.keras.Input(
        shape=input_shape, name="input"
    )

    input_conv = layers.Conv2D(32, (3, 3), activation='relu')(title_input)
    conv = layers.Conv2D(64, (3, 3), activation='relu')(input_conv)
    conv = layers.Conv2D(64, (3, 3), activation='relu')(conv)
    pool = layers.MaxPooling2D((2, 2))(conv)
    drop = layers.Dropout(0.3)(pool)
    conv = layers.Conv2D(128, (3, 3), activation='relu')(drop)
    conv = layers.Conv2D(128, (3, 3), activation='relu')(conv)
    pool = layers.MaxPooling2D((2, 2))(conv)
    drop = layers.Dropout(0.3)(pool)
    conv = layers.Conv2D(256, (3, 3), activation='relu')(drop)
    drop = layers.Dropout(0.5)(conv)
    flat = layers.Flatten()(drop)
    dense = layers.Dense(256, activation='relu')(flat)

    age_dense = layers.Dense(256, activation='relu')(dense)
    ...
    dialect_dense = layers.Dense(256, activation='relu')(dense)
    ...
    gender_dense = layers.Dense(256, activation='relu')(dense)
